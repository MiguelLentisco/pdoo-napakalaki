...
class PruebaNapakalaki
  ...
  def initialize
    ...
    bC = BadConsequence.newLevelSpecificTreasures("Te asusta en la noche. Pierdes
        un casco visible", 1, [TreasureKind::HELMET], [])
  ...
  def justLooseLevels
    @@monsters.select {|monster| monster.badConsequence.nVisibleTreasures == 0 and
        ...
        monster.badConsequence.specificVisibleTreasures == nil and
        monster.badConsequence.specificHiddenTreasures == nil and 
  ...
  def specificTreasures(treasureType)
    
    @@monsters.select {|monster| monster.badConsequence}
    # TODO
    list = []
    for monster in @@monsters
      bc = monster.badConsequence
      found = false
      for treasure in bc.specificHiddenTreasure 
        if bc.specificHiddenTreasures != nil || bc.specificVisibleTreasures != nil
          list << monster
        end
      end
    end
